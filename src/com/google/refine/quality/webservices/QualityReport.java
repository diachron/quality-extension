	...
	public void createQualityReport (URL outputPath, URL inputDataset,  List<QualityProblem>problems){
		...
		report.addProperty(QR.computedOn, model.createResource(inputDataset.toString()));		 
		 for (QualityProblem qualityProblem : problems) {
			 ...
			 Resource poblemTriple = model.createResource(generateURI())
			.addProperty(RDF.type, RDF.Statement)
			 .addProperty(RDF.subject, model.createResource( qualityProblem.getQuad().getSubject().toString()))
			  .addProperty(RDF.predicate, model.createResource(qualityProblem.getQuad().getPredicate().toString())
			   .addProperty(RDF.object, model.createResource( qualityProblem.getQuad().getObject().toString())));
