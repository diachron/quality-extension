...
WebServiceView.prototype = {
		...
		buildHtml : function() {
			...
			for(var metricKey in this.metrics) {
				...
				var item = $('<li><input type="checkbox" value="" id="'+metricKey+'" />' + metric["name"] + '</li>');
				...
				for(var i=0; i<subMetrics.length; i++) {
					...
					var subItem = $('<li><input type="checkbox" value="'+subMetricKey+'" id="'+subMetricKey+'" />'+ subMetric[subMetricKey] +'</li>');
